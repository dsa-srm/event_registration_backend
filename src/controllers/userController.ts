...
export const getUserDetails = async (req: Request, res: Response) => {
	try {
		...
		const events = await db.any(
			"SELECT u.id , u.name  FROM public.registrations r INNER JOIN public.events u ON r.user_event = u.id WHERE r.user_id = $1",
			[id]
		);
...
export const getUserEvents = async (req: Request, res: Response) => {
	try {
		...
		const events = await db.any(
			"SELECT e.id, e.name FROM public.events e INNER JOIN public.registrations r ON e.id = r.user_event WHERE r.user_id = (SELECT id FROM public.users WHERE reg = $1);",
			[reg_no]
		);
